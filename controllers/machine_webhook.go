...
// +kubebuilder:webhook:path=/validate-infrastructure-cluster-x-k8s-io-v1alpha3-machine,mutating=false,failurePolicy=fail,groups=cluster.x-k8s.io,resources=machines,verbs=create,versions=v1alpha3,name=providervalidation.machine.infrastructure.cluster.x-k8s.io
...
func (r *MachineWebhook) Handle(ctx context.Context, req admission.Request) admission.Response {
	machine := &clusterv1.Machine{}

	if err := r.decoder.Decode(req, machine); err != nil {
	...
	object, err := external.Get(ctx, r.client, &machine.Spec.InfrastructureRef, machine.Namespace)
	if err != nil {
		// Allow Machine if AWSMachine doesn't exist
...
func (r *MachineWebhook) InjectClient(c client.Client) error {
	r.client = c
	return nil
}

// InjectDecoder implements admission.DecoderInjector.
func (r *MachineWebhook) InjectDecoder(d *admission.Decoder) error {
	r.decoder = d
	return nil
}
